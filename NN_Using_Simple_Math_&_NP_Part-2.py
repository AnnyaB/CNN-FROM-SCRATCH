...
def sigmoid_derivative(x):
    """
    Calculates the gradient of the Sigmoid function, giving the sensitivity of the output
...
class ProbabilisticNeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size):
        """
        Initializing neural network with input, hidden, and output layers using probabilistic approaches.
    ...
    def backward(self, X, y, learning_rate):
        ...
        This process applies probabilistic concepts to weight adjustments.
        """
        # Error between actual and predicted probabilities
        error = y - self.final_output
        d_output = error * sigmoid_derivative(self.final_output)  # Applying Sigmoid gradient for probabilistic backpropagation

        # Backpropagating to hidden layer
        d_hidden = d_output.dot(self.weights_hidden_output.T) * relu_derivative(self.hidden_output)

        # Probabilistic weight and bias updates
