...
# Defining the ReLU Activation Function (used in the hidden layer)
def relu(x):
    """
...
# Defining the Derivative of the ReLU Function
...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size, output_size):
        """
        Initializing the neural network with sizes for input, hidden, and output layers.
        ...
        # Randomly initializing the weights between input and hidden layers, and between hidden and output layers.
    ...
    def forward(self, X):
        ...
        # Computing the input to the output layer (weighted sum of hidden layer's output + bias)
        ...
        self.final_output = sigmoid(self.final_input)  # Applying Sigmoid activation to the output layer's input

        return self.final_output  # Returning the output of the network
    ...
    def backward(self, X, y, learning_rate):
        ...
        """
        # Calculating the error (difference between the predicted and actual values)
        error = y - self.final_output  # How much the prediction was wrong ?

        # Calculainge the gradient (derivative) of the error with respect to the output layer
        # This tells us how much the output layer needs to change to reduce the error.
        d_output = error * sigmoid_derivative(self.final_output)  # Applying sigmoid derivative to the output layer

        # Calculating the gradient of the error with respect to the hidden layer
        ...
        d_hidden = d_output.dot(self.weights_hidden_output.T) * relu_derivative(
            self.hidden_output)  # Applying ReLU derivative

        # Updating the weights and biases using the gradients calculated above
        self.weights_hidden_output += self.hidden_output.T.dot(
            d_output) * learning_rate  # Updating weights for hidden-output layer
        self.bias_output += np.sum(d_output, axis=0, keepdims=True) * learning_rate  # Updating biases for output layer

        self.weights_input_hidden += X.T.dot(d_hidden) * learning_rate  # Update weights for input-hidden layer
        self.bias_hidden += np.sum(d_hidden, axis=0, keepdims=True) * learning_rate  # Updating biases for hidden layer

    def train(self, X, y, epochs, learning_rate):
        """
        Training the neural network using forward and backward propagation.
        ...
        for epoch in range(epochs):
            self.forward(X)  # Performing forward propagation to compute the model's output
            self.backward(X, y, learning_rate)  # Performing backpropagation to adjust weights and biases
            ...
            if epoch % 1000 == 0:
                ...
                print(f'Epoch {epoch}, Loss: {loss}')  # Printing loss at regular intervals


# Creating the training data (X) and labels (y) for the AND gate
X = np.array([
    [0, 0],  # Input: 0 AND 0
    [0, 1],  # Input: 0 AND 1
    [1, 0],  # Input: 1 AND 0
...
# Creating an instance of the neural network with 2 input neurons, 4 hidden neurons, and 1 output neuron
nn = NeuralNetwork(input_size=2, hidden_size=4, output_size=1)

# Training the network with the training data for 10,000 epochs and a learning rate of 0.1
nn.train(X, y, epochs=10000, learning_rate=0.1)

# After training, using the network to make predictions on the training data
